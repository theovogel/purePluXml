...
<form class="pure-form" action="statiques.php" method="post" id="form_statics">
	...
	if($plxAdmin->aStats) {
		foreach($plxAdmin->aStats as $k=>$v) { # Pour chaque page statique
			...
			if(!plxUtils::checkSite($v['url'])) {
				echo '</td><td>';
				echo '<a class="pure-button" href="statique.php?p='.$k.'" title="'.L_STATICS_SRC_TITLE.'">'.L_STATICS_SRC.'</a>';
				if($v['active']) {
					echo '&nbsp;<a class="pure-button" href="'.PLX_ROOT.'?static'.intval($k).'/'.$v['url'].'" title="'.L_STATIC_VIEW_PAGE.' '.plxUtils::strCheck($v['name']).' '.L_STATIC_ON_SITE.'">'.L_VIEW.'</a>';
				}
				echo '</td></tr>';
			}
			else
				echo '</td><td><a class="pure-button" href="'.$v['url'].'" title="'.plxUtils::strCheck($v['name']).'">'.L_VIEW.'</a></td></tr>';
		}
