...
<form class=" pure-form pure-form-aligned" action="article.php" method="post" id="form_article">

	<div class="pure-g-r">

		<div class="pure-u-2-5 extra-content">

			<fieldset>
	       		<div class="pure-control-group">
	       			<p><label><?php echo L_ARTICLE_STATUS ?>&nbsp;:&nbsp;</label>
						<span class="pure-help-inline">
						<?php
						if(isset($_GET['a']) AND preg_match('/^_[0-9]{4}$/',$_GET['a']))
							echo L_AWAITING;
       			...
       			<div class="pure-control-group">
					<p><label for="id_author"><?php echo L_ARTICLE_LIST_AUTHORS ?>&nbsp;:&nbsp;</label>
					<?php
					...
					<p><label><?php echo L_ARTICLE_DATE ?>&nbsp;:</label>
						<?php plxUtils::printInput('day',$date['day'],'text','1-2',false,'fld1'); ?>
						<?php plxUtils::printInput('month',$date['month'],'text','1-2',false,'fld1'); ?>
						<?php plxUtils::printInput('year',$date['year'],'text','1-4',false,'fld2'); ?>
						<?php plxUtils::printInput('time',$date['time'],'text','1-5',false,'fld2'); ?>
						<a href="javascript:void(0)" class="pure-button" style="padding:5px;" onclick="dateNow(<?php echo date('Z') ?>); return false;" title="<?php L_NOW; ?>"><img src="theme/images/date.png" alt="" /></a>
					</p>
				</div>

       			<div class="pure-control-group">
					<p><label><?php echo L_ARTICLE_CATEGORIES ?>&nbsp;:</label>
        		<div class="pure-controls">
       			...
       			<div class="pure-control-group">
					...
					<p><label for="id_new_catname"><?php echo L_NEW_CATEGORY ?>&nbsp;:</label>
					...
					<p><label for="id_tags"><?php echo L_ARTICLE_TAGS_FIELD ?>&nbsp;:</label><a class="help" class="pure-button" title="<?php echo L_ARTICLE_TAGS_FIELD_TITLE ?>"></a>
					<?php plxUtils::printInput('tags',$tags,'text','25-255'); ?>
					<a class="pure-button" title="<?php echo L_ARTICLE_TOGGLER_TITLE ?>" id="toggler" href="javascript:void(0)" onclick="toggleDiv('tags','toggler','+','-')" style="padding: 5px;">+</a></p>
					...
					if($plxAdmin->aTags) {
						...
						foreach($array as $tagname => $tag) {
							echo '<a class="pure-button" href="javascript:void(0)" onclick="insTag(\'tags\',\''.$tagname.'\')" title="'.plxUtils::strCheck($tagname).' ('.$tag['count'].')">'.plxUtils::strCheck($tagname).'</a> ('.$tag['count'].') ';
						}
					}
					else echo L_NO_TAG;
					?>
				...
				<?php if($plxAdmin->aConf['allow_com']=='1') : ?>
					<p><label for="id_allow_com"><?php echo L_ALLOW_COMMENTS ?>&nbsp;:</label>
					<?php plxUtils::printSelect('allow_com',array('1'=>L_YES,'0'=>L_NO),$allow_com); ?>
					<?php else: ?>
						<?php plxUtils::printInput('allow_com','0','hidden'); ?>
					<?php endif; ?></p>

					<p><label for="id_url"><?php echo L_ARTICLE_URL_FIELD ?>&nbsp;:</label>&nbsp;<a class="help" title="<?php echo L_ARTICLE_URL_FIELD_TITLE ?>">&nbsp;</a>
					<?php plxUtils::printInput('url',$url,'text','27-255'); ?></p>

					<p><label for="id_template"><?php echo L_ARTICLE_TEMPLATE_FIELD ?>&nbsp;:</label>
					<?php plxUtils::printSelect('template', $aTemplates, $template); ?></p>

					<p><label for="id_title_htmltag"><?php echo L_ARTICLE_TITLE_HTMLTAG ?>&nbsp;:</label>
					<?php plxUtils::printInput('title_htmltag',plxUtils::strCheck($title_htmltag),'text','27-255'); ?></p>

					<p><label for="id_meta_description"><?php echo L_ARTICLE_META_DESCRIPTION ?>&nbsp;:</label>
					<?php plxUtils::printInput('meta_description',plxUtils::strCheck($meta_description),'text','27-255'); ?></p>

					<p><label for="id_meta_keywords"><?php echo L_ARTICLE_META_KEYWORDS ?>&nbsp;:</label>
					<?php plxUtils::printInput('meta_keywords',plxUtils::strCheck($meta_keywords),'text','27-255'); ?></p>
				</div>

				<?php eval($plxAdmin->plxPlugins->callHook('AdminArticleSidebar')) # Hook Plugins ?>

				<?php if($artId != '0000') : ?>
					<a class="pure-button" style="padding: 5px;" href="comments.php?a=<?php echo $artId ?>&amp;page=1" title="<?php echo L_ARTICLE_MANAGE_COMMENTS_TITLE ?>"><?php echo L_ARTICLE_MANAGE_COMMENTS ?></a>
					<a class="pure-button" style="padding: 5px;" href="comment_new.php?a=<?php echo $artId ?>" title="<?php echo L_ARTICLE_NEW_COMMENT_TITLE ?>"><?php echo L_ARTICLE_NEW_COMMENT ?></a>
				<?php endif; ?>
		...
		<div class="pure-u-2-5">
			...
			<p class="back"><a class="pure-button" href="index.php"><?php echo L_BACK_TO_ARTICLES ?></a></p>

			<?php eval($plxAdmin->plxPlugins->callHook('AdminArticleTop')) # Hook Plugins ?>

			<div class="form_content">
				<fieldset>
        			<legend><h2><?php echo (empty($_GET['a']))?L_MENU_NEW_ARTICLES:L_ARTICLE_EDITING; ?></h2></legend>
					<?php plxUtils::printInput('artId',$artId,'hidden'); ?>
					<p><label for="id_title"><?php echo L_ARTICLE_TITLE ?>&nbsp;:</label>
					<?php plxUtils::printInput('title',plxUtils::strCheck($title),'text','42-255'); ?></p>
					<p id="p_chapo">
						<label for="id_chapo"><?php echo L_HEADLINE_FIELD ?>&nbsp;:</label>
						&nbsp;
						<a class="pure-button" id="toggler_chapo" href="javascript:void(0)" onclick="toggleDiv('toggle_chapo', 'toggler_chapo', '<?php echo L_ARTICLE_CHAPO_DISPLAY ?>','<?php echo L_ARTICLE_CHAPO_HIDE ?>')"><?php echo $chapo==''?L_ARTICLE_CHAPO_DISPLAY:L_ARTICLE_CHAPO_HIDE ?></a>
					</p>
					...
					<?php if($artId!='' AND $artId!='0000') : ?>
						...
						<p id="p_link"><label for="id_link"><?php echo L_LINK_FIELD ?>&nbsp;:&nbsp;</label>
							<?php echo '<a class="pure-button" onclick="this.target=\'_blank\';return true;" href="'.$link.'" title="'.L_LINK_ACCESS.'">'.L_LINK_VIEW.'</a>'; ?>
						</p>
				...
				<p class="center">
					...
					<?php
						if($_SESSION['profil']>PROFIL_MODERATOR AND $plxAdmin->aConf['mod_art']) {
							...
							if($artId!='0000')
								echo '<input class="delete" type="submit" name="delete" value="'.L_DELETE.'" onclick="Check=confirm(\''.L_ARTICLE_DELETE_CONFIRM.'\');if(Check==false) {return false;} else {this.form.target=\'_self\';return true;}" /> ';
							if(in_array('draft', $catId)) {
								...
								echo '<input class="notice submit" onclick="this.form.target=\'_self\';return true;" type="submit" name="publish" value="' . L_ARTICLE_PUBLISHING_BUTTON . '"/> ';
							} else {
								if(preg_match('/^_[0-9]{4}$/',$_GET['a']))
									echo '<input class="notice button submit" onclick="this.form.target=\'_self\';return true;" type="submit" name="publish" value="' . L_ARTICLE_PUBLISHING_BUTTON . '"/> ';
								else
