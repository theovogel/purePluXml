...
<form class="pure-form" action="parametres_plugins.php" method="post" id="form_plugins">
...
<thead>
	<tr>
		...
		<th class="icon">&nbsp;</th>
		<th class="description"><a class="pure-button pure-button-primary" title="<?php echo L_PLUGINS_ALPHA_SORT ?>" href="parametres_plugins.php?sort"><?php echo L_MENU_CONFIG_PLUGINS ?></a></th>
		<th class="col"><a class="pure-button pure-button-primary" href="parametres_plugins.php"><?php echo L_PLUGINS_LOADING_SORT ?></a></th>
...
<tbody>
	...
	if(sizeof($tmp)>0) {
		...
		foreach($plxAdmin->plxPlugins->aPlugins as $plugName => $plugAttrs) {
			...
			if($plugin->getInfo('site')!='') echo ' <a class="pure-button" href="'.plxUtils::strCheck($plugin->getInfo('site')).'">'.plxUtils::strCheck($plugin->getInfo('site')).'</a>';
			echo '</td>';
			...
			echo '<a class="pure-button" title="'.L_PLUGINS_HELP_TITLE.'" href="parametres_pluginhelp.php?p='.urlencode($plugName).'">'.L_PLUGINS_HELP.'</a>';
			# affichage du lien pour configurer le plugin
			if(is_file(PLX_PLUGINS.$plugName.'/config.php'))
			echo '&nbsp;<a class="pure-button" title="'.L_PLUGINS_CONFIG_TITLE.'" href="parametres_plugin.php?p='.urlencode($plugName).'">'.L_PLUGINS_CONFIG.'</a>';
