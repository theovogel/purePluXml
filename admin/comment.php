...
if(($aFile = $plxAdmin->plxGlob_arts->query('/^'.$artId.'.(.+).xml$/','','sort',0,1)) == false) {
	...
	$article = '<a class="pure-button" class="pure-button" href="'.$plxAdmin->aConf['racine'].'index.php?article'.intval($result['numero']).'/'.$result['url'].'" title="'.L_COMMENT_ARTICLE_LINKED_TITLE.'">';
...
elseif($com['comStatus']=='')
	$statut = '<a class="pure-button" class="pure-button" href="'.PLX_ROOT.'?article'.intval($plxAdmin->plxRecord_coms->f('article')).'/#c'.$plxAdmin->plxRecord_coms->f('numero').'" title="'.L_COMMENT_ONLINE_TITLE.'">'.L_COMMENT_ONLINE.'</a>';
else
	$statut = '';
...
<?php if(!empty($_GET['a'])) : ?>
	<p class="back"><a class="pure-button" href="comments.php?a=<?php echo $_GET['a'] ?>"><?php echo L_BACK_TO_ARTICLE_COMMENTS ?></a></p>
<?php else : ?>
	<p class="back"><a class="pure-button" href="comments.php"><?php echo L_BACK_TO_COMMENTS ?></a></p>
<?php endif; ?>
...
<form action="comment.php<?php echo (!empty($_GET['a'])?'?a='.plxUtils::strCheck($_GET['a']):'') ?>" method="post" id="form_comment">
	<fieldset>
		...
		<a class="pure-button" href="javascript:void(0)" onclick="dateNow(<?php echo date('Z') ?>); return false;" title="<?php L_NOW; ?>"><img src="theme/images/date.png" alt="" /></a>
		...
		<?php
			...
			if($site != '')	echo '<a class="pure-button" href="'.$site.'">'.$site.'</a>';
		?>
		...
		<?php echo '<a class="pure-button" href="mailto:'.$plxAdmin->plxRecord_coms->f('mail').'">'.$plxAdmin->plxRecord_coms->f('mail').'</a>' ?>
		<?php endif; ?>
